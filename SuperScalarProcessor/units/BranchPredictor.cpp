...
void BranchPredictor::setPrediction(const PC &cur_pc, const PC &pc_disp, bool comp) {
    ...
    if (/* bit valid */ bht_bucket1[bht_bucket_size - 1] && tag1 == tag) {
        updatePrediction(bht_bucket1, key.to_ulong(), comp);
    } else if (/* bit valid */ bht_bucket2[bht_bucket_size - 1] && tag2 == tag) {
        updatePrediction(bht_bucket2, key.to_ulong(), comp);
...
void BranchPredictor::updatePrediction(std::bitset<bht_bucket_size> &bht_bucket, uint32_t key, bool comp) {
...
void BranchPredictor::setupBucket(std::bitset<bht_bucket_size> &bht_bucket, std::bitset<btb_bucket_size> &btb_bucket,
                                  uint32_t key, bool comp, std::bitset<32> target,
                                  std::bitset<30 - key_size> tag) {
...
bool BranchPredictor::getPrediction(const PC &cur_pc) {
    std::bitset<32> pc{cur_pc.realVal()};
    std::bitset<key_size> key = sub_range<key_size + 1, 2>(pc);
    auto tag = sub_range<31, key_size + 2>(pc);
    auto &bht_bucket1 = bht_[key.to_ulong()].first;
    ...
    if (/* bit valid */ !bht_bucket1[bht_bucket_size - 1] && !bht_bucket2[bht_bucket_size - 1]) {
        return false;
    } else if (/* bit valid */ bht_bucket1[bht_bucket_size - 1] && tag == tag1) {
...
std::bitset<32> BranchPredictor::getTarget(bool pred, const PC &cur_pc) {
    if (!pred) {
        return std::bitset<32>{cur_pc.realVal()};
    }
    
    std::bitset<32> pc{cur_pc.realVal()};
    auto tag = sub_range<31, key_size + 2>(pc);
    std::bitset<key_size> key = sub_range<key_size + 1, 2>(pc);
    auto &btb_bucket1 = btb_[key.to_ulong()].first;
    auto &btb_bucket2 = btb_[key.to_ulong()].second;
